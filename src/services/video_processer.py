...
class VideoProcesser:
    ...
    def send_data(self, tray_id: int, bread_amount: list[int], detected_time: datetime) -> None:
        """
    ...
    def process_inter_data(self, is_covered: int, bread_amount: list[list[int]], tray_id: int, start_roi: np.ndarray[Any],
                           finish_roi: np.ndarray[Any], model: YOLO, detected_time: datetime, roi: np.ndarray[Any]) -> tuple[
            int, list, int]:
        """
        ...
        if np.count_nonzero(start_roi == 0) / (start_roi.shape[0] * start_roi.shape[1]) >= 0.6 and np.count_nonzero(
            ...
            if is_covered < 11 and bread_amount != [[], [], [], [], [], [], [], [], [], [], [], []]:
                ...
                if bread_amount[0] > 0 and sum(bread_amount[1: 11]) < 12:
                    self.send_data(tray_id=tray_id, bread_amount=bread_amount,
                                   detected_time=detected_time)
                    tray_id += 1
            is_covered = 0
            bread_amount = [[], [], [], [], [], [], [], [], [], [], [], []]
    ...
    def cut_grayscale_roi(self, x1: int, y1: int, x2: int, y2: int, roi: np.ndarray[Any]) -> np.ndarray[Any]:
        """
        ...
        roi[(roi[:, :, 0] > roi[:, :, 1]) & (
            roi[:, :, 0] > roi[:, :, 2])] = (255, 255, 255)
        roi = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
        roi[roi != 255] = 0

        return roi.copy()

    def process_video(self, model: any, cap: any, ms_per_frame: float, detected_time: datetime, is_covered: any,
                      bread_amount: any,
                      tray_id: int) -> int:
        """
        Process video.
        ...
        while True:
            ...
            is_covered, bread_amount, tray_id = self.process_inter_data(is_covered, bread_amount, tray_id, start, finish,
    ...
    def run(self, model, tray_id: int) -> None:
        ...
        while True:
            ...
            if datetime.now() - timedelta(seconds=self.config.video_length_s) <= detected_time:
                continue
            ...
            tray_id = self.process_video(model, cap, ms_per_frame,
            ...
            logger.info(
                f"File {self.config.video_path}/{video} has been removed")
